...
$VMName = "VM-Server1"
$DiskSizeGB = 32
$AzSubscription = "Demo Subscription"
$Location = "westeurope"
...
$destinationVHDFileName = "DiskNameForSnapshotCopy.vhd"
...
New-AzStorageContainer -Name $storageContainerName -Permission Off -Context $destinationContext
...
$emptydiskforfootername = "DiskNameForSnapshotCopy-empty.vhd"

# Empty disk URI
#$EmptyDiskURI = $container.CloudBlobContainer.Uri.AbsoluteUri + "/" + $emptydiskforfooter

$diskConfig = New-AzDiskConfig -Location $Location -CreateOption Empty -DiskSizeGB $DiskSizeGB -HyperVGeneration $HyperVGen
...
$datadisk = Get-AzStorageBlob -Context $destinationContext -Container $storageContainerName -Blob $destinationVHDFileName

$footer = New-Object -TypeName byte[] -ArgumentList 512
write-output "Get footer of empty disk"

$emptyDiskblob.ICloudBlob.DownloadRangeToByteArray($footer, 0, $emptyDiskblob.Length - 512, 512)

$dataDisk.ICloudBlob.Resize($emptyDiskblob.Length)
$footerStream = New-Object -TypeName System.IO.MemoryStream -ArgumentList (,$footer)
write-output "Write footer of empty disk to DataDisk"
$dataDisk.ICloudBlob.WritePages($footerStream, $emptyDiskblob.Length - 512)
...
$vhdUri = $datadisk.ICloudBlob.Uri.AbsoluteUri

# Specify the disk options
$diskConfig = New-AzDiskConfig -AccountType $accountType -Location $Location -DiskSizeGB $DiskSizeGB -SourceUri $vhdUri -CreateOption Import -StorageAccountId $StorageAccount.Id -HyperVGeneration $HyperVGen

#Create Managed disk
New-AzDisk -DiskName $NewDiskName -Disk $diskConfig -ResourceGroupName $resourceGroupName

# Delete old Managed Disk
Remove-AzDisk -ResourceGroupName $resourceGroupName -DiskName $DiskName -Force;

# Delete old blob storage
$datadisk | Remove-AzStorageBlob -Force
